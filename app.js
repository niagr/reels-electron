    ...
    (function (fs_1) {
        function init() {
            if (Platform.platform == 'WinRT') {
                ...
                fs_1.chooseEntry = chooseEntryElectron;
                fs_1.appDataDir = appDataDirElectron;
                fs_1.retainEntry = retainEntryNW;
                fs_1.restoreEntry = restoreEntryNW;
                fs_1.openFileWithSystemDefault = openFileWithSystemDefaultElectron;
                fs_1.openURLWithSystemDefault = openURLWithSystemDefaultElectron;
            }
        }
        fs_1.init = init;
        /**
        ...
        function chooseEntryElectron(chooser_type, success, failure) {
            ...
            dialog.showOpenDialog(props, function (selected) {
                var path = require('path');
        ...
        function openFileWithSystemDefaultElectron(file) {
            require('electron').shell.openItem(file.get_full_path());
        }
        ...
        function openURLWithSystemDefaultElectron(url) {
            require('electron').shell.openExternal(url);
        }
        function appDataDirNodeCommon(appDataDirPath, callback) {
            var path = require('path');
            var fs = require('fs');
            var data_dir = appDataDirPath;
        ...
        function appDataDirElectron(callback) {
            var path = require('path');
            var data_dir = path.join(require('electron').remote.app.getPath('userData'), "appdata");
            appDataDirNodeCommon(data_dir, callback);
        }
